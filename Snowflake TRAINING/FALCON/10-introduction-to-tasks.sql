...
CREATE SCHEMA IF NOT EXISTS FALCON_TASKS_SCHEMA;
USE SCHEMA FALCON_TASKS_SCHEMA;
...
AS    
    INSERT INTO FALCON_DB.FALCON_TASKS_SCHEMA.SHIPPING_BY_DATE (ORDERDATE, SHIPDATE, DAYSTOSHIP)
    SELECT 
        O_ORDERDATE
        , L_SHIPDATE
    ...
    WHERE 
        YEAR(O_ORDERDATE) = (SELECT yr FROM FALCON_db.FALCON_tasks_schema.year_month WHERE ID = (select MAX(ID) from year_month))
        AND 
        MONTH(O_ORDERDATE)= (SELECT mon FROM FALCON_db.FALCON_tasks_schema.year_month WHERE ID = (select MAX(ID) from year_month));
...
AS  
    INSERT INTO FALCON_DB.FALCON_TASKS_SCHEMA.AVG_SHIPPING_IN_DAYS(yr, mon, avg_shipping_days)
    SELECT
        YEAR(orderdate) AS yr
      , MONTH(orderdate) AS mon
      , AVG(daystoship) AS avg_shipping_days
    FROM
        FALCON_DB.FALCON_TASKS_SCHEMA.SHIPPING_BY_DATE
    WHERE
        yr = (SELECT yr FROM FALCON_db.FALCON_tasks_schema.year_month WHERE ID = (select MAX(ID) from year_month))
        AND
        mon = (SELECT mon FROM FALCON_db.FALCON_tasks_schema.year_month WHERE ID = (select MAX(ID) from year_month))
    GROUP BY 
        YEAR(orderdate)
      , MONTH(orderdate);
...
$$
    var r='continue';
    var mon = snowflake.createStatement({sqlText: 'SELECT mon FROM FALCON_db.FALCON_tasks_schema.year_month WHERE ID = (select MAX(ID) from year_month);'});
...
DROP SCHEMA FALCON_TASKS_SCHEMA;
