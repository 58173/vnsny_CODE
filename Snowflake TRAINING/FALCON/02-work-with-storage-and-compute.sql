...
-- 2.2.2   Locate and click FALCON_DB in the database list.
...
-- 2.2.11  Create a schema in your database, and name it FALCON_SCHEMA.
...
CREATE SCHEMA FALCON_DB.FALCON_SCHEMA;
...
-- 2.5.2   Using SQL, create a warehouse named FALCON_WH2, with the following
--         characteristics.
...
-- 2.5.15  Locate FALCON_WH2 and verify the Status is now Started.
...
-- 2.5.17  Suspend FALCON_WH2:

ALTER WAREHOUSE FALCON_WH2 SUSPEND;
...
-- 2.5.19  Alter FALCON_WH2 and set the WAREHOUSE_SIZE to XSmall:

ALTER WAREHOUSE FALCON_WH2 SET WAREHOUSE_SIZE=XSmall;


-- 2.5.20  Suspend and then drop FALCON_WH2:
...
--         FALCON_WH2 cannot be suspended.
--         Proceed with the statement below to drop the warehouse:

DROP WAREHOUSE FALCON_WH2;


-- 2.5.21  Show warehouses to verify that FALCON_WH is still listed, but
--         FALCON_WH2 is not.
